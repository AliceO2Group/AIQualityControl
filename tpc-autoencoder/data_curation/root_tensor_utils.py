...
def save_root_comparison(
    ...
    # Use original display range on both data panels for a fair visual comparison
    zmin = float(info.display_min) if info.display_min is not None else float(th2_orig.GetMinimum())
    zmax = float(info.display_max) if info.display_max is not None else float(th2_orig.GetMaximum())
    th2_orig.SetMinimum(zmin)
    th2_orig.SetMaximum(zmax)
    th2_reco.SetMinimum(zmin)
    th2_reco.SetMaximum(zmax)
    ...
    # save as both PNG (quick view) and ROOT (open in ROOT browser)
